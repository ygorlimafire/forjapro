...
export function ProductActions({ productId, isActive }: Props) {
  ...
  async function handleToggle() {
    ...
    if (result.success) {
      toast.success(isActive ? "Produto inativado" : "Produto ativado")
      router.refresh()
    } else {
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem onClick={() => router.push(`/produtos/${productId}`)}>
            <Eye size={14} /> Ver detalhes
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => router.push(`/produtos/${productId}/editar`)}>
